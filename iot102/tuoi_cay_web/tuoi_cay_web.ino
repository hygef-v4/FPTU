...
// ### Initialize Hardware ###
LiquidCrystal_I2C lcd(0x27, 16, 2);  // LCD I2C address, column, row
DHT dht(DHTPIN, DHTTYPE);            // DHT sensor instance
ESP8266WebServer server(80);         // Web server on port 80

// ### WiFi Credentials ###
const char* ssid = "POCO F4";
const char* password = "kurogane123";
...
String buildFancyBar(float value, const String& color) {
  ...
  String barHtml = "<div class='bar-container'>"
                   "  <div class='bar-fill' style='background:"
                   + color + "; width:" + String(value, 1) + "%;'></div>"
                                                             "  <div class='bar-label'>"
                   + String(value, 1) + "%</div>"
                                        "</div>";
  return barHtml;
...
void loop() {
  ...
  // Update sensor data (LCD) every sensorInterval
  ...
  if (autoMode && (currentMillis - lastAutoCheck >= sensorInterval)) {
    ...
    if (!pumpState) {
      ...
      if (soilValue < soilThreshold) {
        ...
        // check if soil is wet enough to stop 
        if (soilValue >= soilThreshold) {
          digitalWrite(PUMP_PIN, HIGH);  // Pump OFF
          pumpState = false;
          Serial.println("Auto Mode: Pump OFF because soil moisture is sufficient");
        }
      }
    }
