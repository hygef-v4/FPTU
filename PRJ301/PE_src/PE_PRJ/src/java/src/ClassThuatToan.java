...
public class ClassThuatToan {
    ...
    /*
    find list of common divisor 
     */
    public static List<Integer> timUocChung(int a, int b) {
    ...
    public static int findGCDOfArray(int[] arr) {
        ...
        for (int i = 1; i < arr.length; i++) {
            ...
            if (result == 1) {
                break; // No need to check further if GCD is 1
            }
        }
        return result;
    }

    // Function to find the common divisors of an array of numbers
    public static List<Integer> findCommonDivisors(int[] arr) {
        int gcdValue = findGCDOfArray(arr);
        List<Integer> divisors = new ArrayList<>();
    ...
    for (String word : words) {
            ...
            if (Character.isLetter(c) && lowerC != 'a' && lowerC != 'e' && lowerC != 'i' && lowerC != 'o' && lowerC != 'u') {
                count++; // Count only consonants
            }
        }
    }
        ...
        int count = 0;
                    ...
                    // tach dau phay ra khoi array 
                    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        PrintWriter out = response.getWriter();
        String[] numbers = request.getParameter("arr").split(",");
        ...
        try {
            for (String number : numbers) {
                ...
                if (num <= 0) {
                    out.println("<h1>" + "You must input an integer array" + "</h1>");
                    return;
                }
                arrList.add(num);
            }
            int sum = 0;
            for (int i : arrList) {
                if (option.equals("even") && i % 2 == 0) {
                    sum += i;

                } else if (option.equals("odd") && i % 2 != 0) {
                    sum += i;
                }
            }
            out.println("<h1>" + "output: " + sum + "</h1>");
            return;
